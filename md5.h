...
private:
    ...
    const unsigned int shiftAmount_s[64] =
    {
    ...
    const std::uint32_t sinesOfIntegers_K[64] =
    {
...
public:
    ...
    std::string textToBinary(std::string);
    void MD5Hash();
    void hashString(std::string);
    void genDigest();
    std::string getDigest();
	void resetVariables();
	std::string maskString(std::string, int, int);
	void formatInput();
